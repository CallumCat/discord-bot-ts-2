    ...
    async run(p: ICommandPayload): Promise<void> {
        if (!p.args[0]) {
            ...
            if (!p.args[1]) {
                ...
                await p.msg.channel.send(MessageBuilder.build({ emoji: "<:yes:602761121602207744>", message: "Commands disabled."}));
            } else {
                await GlobalVars.db.collection(config.db.optionsCollection).updateOne({}, { $set: {commandsEnabled: true}});
                await p.msg.channel.send(MessageBuilder.build({ emoji: "<:yes:602761121602207744>", message: "Commands enabled."}));
            }
        }
        else {
